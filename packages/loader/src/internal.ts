...
export namespace ModuleLoader {
  ...
  function requireInternal(id: string): any {
    ...
    // MODULE_NOT_FOUND appends a require stack that names this file, which
    // tells the user nothing; the reason is the first line
    const detail = (error: unknown) => {
      const message = error instanceof Error ? error.message : String(error)
      return message.split('\n')[0]
    }
    if (process.execArgv.some(arg => arg === '--expose-internals' || arg === '--expose_internals')) {
      try {
        return require(id)
      } catch (error) {
        _failures.push(`require(${JSON.stringify(id)}) with expose-internals: ${detail(error)}`)
      }
    }
    try {
      return require('node-addon-require-builtin').requireBuiltin(id)
    } catch (error) {
      _failures.push(`node-addon-require-builtin: ${detail(error)}`)
    }
  }
  ...
  /**
   * Returns why the last {@link fromInternal} probe produced no loader,
   * so the loader service can report it once at startup.
   */
