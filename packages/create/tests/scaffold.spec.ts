...
// A wait that outlives the progress interval stays visible instead of reading
// as a hang.
it('reports progress while waiting for a slow registry', async () => {
  const server = createServer(() => { /* the request is accepted and left open */ })
  await new Promise<void>((ready) => server.listen(0, '127.0.0.1', () => ready()))
  const registry = `http://127.0.0.1:${(server.address() as { port: number }).port}`
  const cwd = await mkdtemp(join(tmpdir(), 'cordis-create-'))
  try {
    await writeFile(join(cwd, '.npmrc'), `registry=${registry}\n`)
    // The deadline sits just past the 10s progress interval.
    await writeFile(join(cwd, 'entry.mjs'), [
      `import scaffold from ${JSON.stringify(pathToFileURL(resolve(testDir, '../src/index.ts')).href)}`,
      `scaffold({ name: 'cordis', version: '0.0.0', template: '@cordisjs/boilerplate', timeout: 11000 })`,
    ].join('\n'))
    const env = childEnv({
      npm_config_registry: registry,
      npm_config_user_agent: 'npm/10.0.0 node/v22.0.0',
    ...
    const output = `${stdout}${stderr}`
    expect(output).toContain(`Still waiting for ${registry}/@cordisjs/boilerplate (10s)`)
    expect(code).toBe(1)
  } finally {
    await rm(cwd, { recursive: true, force: true })
    server.close()
